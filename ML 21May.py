...
# # Now lets get started with analyzating the supply chain by looking at the relationship b/w the price of the products and the revenue generated by them
...
# # Thus,the company derives more revenue from skincare products, and the higher the price of skincare products, the more revenue they generate . Now lets have a look at the sales by product type:
...
result.rename(columns={'Lead time':'Average Lead Time',
                       'Manufacturing costs':'Average Manufacturing'},inplace=True)
print(result)
...
# # Now lets have a look at the order quantity of each SKUs
...
order_quantity_chart=px.bar(df,x='SKU',
                           y='Order quantities',
                           title='Order Quantitities by SKU')
